...
```{r}
ten.day.high <- c(58, 41, 43, 45, 35, 42, 49, 52, 56, 66)
ten.day.low <- c(31, 19, 25, 27, 18, 23, 25, 29, 43, 49)
```
...
ten.day.high.celcius <- celcius(ten.day.high)
ten.day.low.celcius <- celcius(ten.day.low)

# this is how John did it
f_to_c = function(f){
  c <- (f-32)*5/9
  return(c)
}
...
```{r}
df.durham.weather <- data.frame("F Highs" = ten.day.high, "F Lows" = ten.day.low, "C Highs" = ten.day.high.celcius, "C Lows" = ten.day.low.celcius)

## this is John's way
the_temps <- cbinds(highs_F, lows_F, highs_C, lows_C)
# then assigned the names to the columns
```
...
```{r}
summary(df.durham.weather)
# you can't take the sd of the entire dataset; it doesn't recognize
#character versus numeric things
sd(df.durham.weather$F.Highs)
```
