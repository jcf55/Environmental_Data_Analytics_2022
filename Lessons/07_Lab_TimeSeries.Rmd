...
Rainfall_BR_processed <-
  ...
  #the reason that there is a " " outside of the list is to get rid
  #of the space infront of month name and doesn't create new column
  separate(Statistics,c("Null","Month","Null2")," ") %>% 
  #then you create a date object; paste them together 03-1998
  #the prefix my() turns it into a lubridate date function
  mutate( Date = my(paste0(Month,"-",Year))) %>% 
  #then get rid of everything else that we don't want
...
#summary tells you if there are NAs
summary(Temp_BR_processed)
summary(Rainfall_BR_processed)
#this also works but this returns a longer output
...
rainfall_df <- as.data.frame(rainfall_decomp$time.series[,1:3])

temperature_df <- as.data.frame(temperature_decomp$time.series[,1:3])

BR_Rain_Components <- mutate(rainfall_df,
                             Observed = BRA_complete$Rainfall,
                             Date = BRA_complete$Date)
BR_Temp_Components <- mutate(temperature_df,
...
ggplot(BR_Temp_Components) +
  ...
  #geom_line(aes( y = seasonal, x = Date), color = "blue") +
  ylab("Rainfall")
  #if you wanted horizontal line geom_hline(yintercept = 0, lty = 2)
