...
the_registrant <- webpage %>% 
  #the below value in "" is grabbed from the extractor plug in from the webpage; go look in this website item and look for the items with this descriptor
  html_nodes(".table tr:nth-child(1) td:nth-child(2)") %>% 
  #now that you have this value, now give me the associated text
  html_text()
the_registrant
```
...
#to get this from multiple places just control click on the item of interest and then click on other items to make them red and then control click on the next item of interest and once it turns green as well you have eliminated enough things fort the plug in to select other items
...
df_withdrawals <- data.frame("Month" = rep(1:12), #create list of 1 - 12
                             "Year" = rep(2000,12), #twelve instances of 2000
...
df_withdrawals <- df_withdrawals %>% 
  mutate(Registrant = !!the_registrant, #!! say that this is a variable not a name
         Facility_name = !!the_facility_name,
         Facility_type = !!the_facility_type,
         Date = my(paste(Month,"-",Year))) #this addes the date
...
#we have to look at what changing things in the URL does to figure it out
#all you have to do is change the site ID and the year to get more information; make sure not to hard code but use variable names
the_base_url <- 'https://www.ncwater.org/WUDC/app/WWATR/report/view'
the_facility <- '0218-0238'
the_year <- 2015
#then create an object that takes our variables and creates it into a string; paste0 eliminates the spaces between concatinated values
the_scrape_url <- paste0(the_base_url, '/', the_facility, '/', the_year)
print(the_scrape_url)

#Retrieve the website contents; read_html()
...
#this only creates the function doesn't create an output
scrape.it <- function(the_year, the_facility){
  ...
  #by having this(change number to change # of seconds) you can prevent the website from thinking that it's under attack. You could lose access to the site (or for Duke at large), this has to do with ethics
...
#repeat the function for each value in the array and specifying the facility right here
the_dfs <- lapply(X = the_years,
                  FUN = scrape.it,
                  the_facility=my_facility)

#OR, use purrr's map function (this is from the tidyverse)
...
#before we used html_text which printed text (in this case "view report") but html_attr will print the web address associated with the report (the attribute)
...
#the above line cuts down the list of 800+ items that are returned
#Have a look: 
...
#we have to create this list of the years because otherwise you're not iterating through values that are the same size
...
#cross is for when you want each year for each site (basically creating a 3X3 list aka 9 items)
the_df <- cross2(the_years,the_facilities) %>% 
  map(lift(scrape.it)) %>% #created list, run through the function, and binding the rows below
  bind_rows()
