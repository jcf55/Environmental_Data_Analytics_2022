...
NTL.phys.data.Tues90.99 <- filter(NTL.phys.data, year4 %in% (1990:1999) &
                                    lakename == "Tuesday Lake")

```
Question: Why don't we filter using row numbers?

> Answer: Row numbers are arbitrary and can change and may not include the
...
Ex4 <- NTL.phys.data.pipe %>% 
  filter(depth == 0) %>% 
  filter(!is.na(temperature_C)) %>%  #alt dropna(temperature_C)
