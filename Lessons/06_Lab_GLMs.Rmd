...
> Answer: The null hypothesis is that the means of the AQIs are not distinctly different. The alternative hypothesis is that the means of the AQIs are distinctly different based on a given confidence interval. 
...
ggplot(EPAair, aes(x= as.factor(Year), y = Ozone)) +
  geom_boxplot() +
  #this is how you get labels to show they are different
...
```{r}
#sum variance - total variance = adjusted R^2 value. The smaller the value, the less variance there is. 
# p-value states the strength of the relationship between the variables

#Exercise 3: Run a linear regression PM2.5 by Ozone. Find the p-value and R-squared value. 
pm.oz.lm <- lm(EPAair$PM2.5 ~ EPAair$Ozone) #ozone is second b/c it's the independent variable
#summary
summary(pm.oz.lm)
...
ggplot(EPAair, aes(x = Ozone, y = PM2.5)) +
  geom_point() + 
  geom_smooth(method = "lm") #if you don't want standard error se = FALSE 
```
...
the_lm <- lm(data = EPAair, PM2.5 ~ Ozone + SITE_LATITUDE + SITE_LONGITUDE + Year + Month)
summary(the_lm)

step(the_lm) #the final output from this is the lm we want to use
...
the_lm_revised <- lm(data = EPAair, PM2.5 ~ Ozone + Month + SITE_LATITUDE + SITE_LONGITUDE)
summary(the_lm_revised) #higher degrees of freedom so this is better
```
