...
# Graph the results- angled text on axis in this code
...
> See differences in the mean from the graph. We would want to order this graph so that all the A, B, C, D groups are aligned. This way we could visually compare items. 
...
Litter.anova.plot <- ggplot(Litter, aes(y = dryMass, x = nlcdClass, color = functionalGroup)) +
  geom_boxplot() +
  stat_summary(geom = "text",label = c('a', 'b', 'bc', 'cd', 'd'))
