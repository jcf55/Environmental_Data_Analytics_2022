...
svi2018_nc_raw <- read.csv(
  ...
  colClasses = c('FIPS' = 'factor')) %>%  #FIPS should be factor or characters not numbers
...
#Grab the intersecting HUC_8s (see two ways)
triangle_HUCs <- huc8_sf_utm %>%
  filter(st_intersects(x = ., y = triangle_co, sparse = F))
triangle_HUCs <- huc8_sf_utm[triangle_co,] #this is matrix notation
