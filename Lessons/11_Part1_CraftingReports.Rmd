...
author: Jackie Fahrenholz - Environmental Data Analytics | Developed by Kateri Salk
date: "Spring 2022"
output:
...
* Directly formats code into documents
* Can create multiple forms of output (i.e. powerpoints, pdfs, htmls)
* Great for team collaboration
* Much neater than just using R code files, since you'd need to use comments
...
#load in the data and format it properly
lake_nutrient <- read_csv("../Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv")
head(lake_nutrient)
lake_nutrient$sampledate <- as.Date(lake_nutrient$sampledate, 
                                     format = "%m/%d/%y")
```
...
lake_wrangle <-
  ...
  select(lakename, year4, daynum, sampledate, depth, tn_ug, tp_ug, 
         nh34, no23, po4) %>%
  filter(depth == 0) %>%
  filter(!is.na(tn_ug) & !is.na(depth) 
         & !is.na(daynum) & !is.na(year4) & !is.na(tp_ug) & !is.na(nh34) & !is.na(no23) & !is.na(po4))
```
...
```{r summarize, message = FALSE, include = FALSE}
lake_sum_nit <-
  lake_wrangle %>%
  group_by(lakename) %>%
  summarise(mean_nit = mean(tn_ug),
            min_nit = min(tn_ug),
            max_nit = max(tn_ug),
            sd_nit = sd(tn_ug))
...
lake_sum_phos <-
  lake_wrangle %>%
  group_by(lakename) %>%
  summarise(mean_phos = mean(tp_ug),
            min_phos = min(tp_ug),
            max_phos = max(tp_ug),
            sd_phos = sd(tp_ug))
```
...
```{r table building, echo=FALSE, fig.align = 'left'}
table_nit <- kable(lake_sum_nit, caption = "Total Nitrogen Summary")
table_nit
```

``` {r table building 2, echo = FALSE, fig.align = 'left'}
table_phos <- kable(lake_sum_phos, caption = "Total Phosphorous Summary")
table_phos
```
...
```{r total N display, echo = FALSE, warning = FALSE, fig.align = 'left', fig.cap = 'Summary of Total N'}
ggplot(lake_wrangle, aes (x = sampledate, y = tn_ug, color = lakename)) +
  geom_point()+
...
```{r total P display, echo = FALSE, warning = FALSE, fig.align = 'left', fig.cap = 'Summary of Total P'}
ggplot(lake_wrangle, aes (x = sampledate, y = tp_ug, color = lakename)) +
  geom_point()+
  geom_smooth(method = "loess", se = FALSE)+
  xlab('Date') +
...
From this exercise, we find that not all lakes have complete datasets for evaluating nutrient data taken across all sample dates. Because of this, when summaries are created, not all lakes can be evaluated. When comparing the average total nitrogen and total phosphorus we see that the order in which lakes fall differs between each nutrient. Mean phosphorus levels are highest in Hummingbird lake and lowest in Paul lake. At the same time, mean nitrogen levels are West Long lake has the highest nitrogen values, while Crampton lake has the lowest. There is more variation between the phosphorus samples (large differences in standard deviation) than is seen in nitrogen samples.

***

This leads us in the direction of seeing if any of the nutrients follow a similar trend to one another. We should attempt to compare all nutrient types and see if one can be used as a predictor of another. We would need to identify if we want to take these samples all as if they come from the same lake, or if we wanted to group the data by lake name as these would lead to two different analyses. The first would be a generalized linear models, while the second could lead to an ANOVA. We could also look at more in depth, as we started to do this in class already, if there is seasonality in all of the variables as it could be correlated with time. This would lead us to use time series analysis methods.
